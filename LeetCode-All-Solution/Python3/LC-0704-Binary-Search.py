...
class Solution:
    ...
    #     n_len = len(nums)
    #     start_index, end_index = 0, n_len
    #     cur_index = n_len >> 1
    #     while start_index <= cur_index < end_index:
    #         cur_num = nums[cur_index]  # cache variable
    #         if start_index == (end_index - 1):  # border case: length of interval is 1
    #             return cur_index if cur_num == target else -1
    #         if cur_num == target:  # 1. hit the target
    #             return cur_index  # bingo
    #         elif cur_num < target:  # 2. go right
    #             start_index = cur_index  # change interval
    #             cur_index = (end_index + start_index) >> 1  # change current cursor
    #         else:  # 3. go left
    #             end_index = cur_index  # change interval
    #             cur_index = (end_index + start_index) >> 1  # change current cursor
    #     return -1
