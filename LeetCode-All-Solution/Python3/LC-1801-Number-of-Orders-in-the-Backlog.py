...
@File    : LC-1801-Number-of-Orders-in-the-Backlog.py
...
"""
LeetCode - 1801 - (Medium) - Number of Orders in the Backlog
https://leetcode.com/problems/number-of-orders-in-the-backlog/

Description & Requirement:
    You are given a 2D integer array orders, where each orders[i] = [price_i, amount_i, orderType_i] denotes that 
    amount_i orders have been placed of type orderType_i at the price price_i. The orderType_i is:
        0 if it is a batch of buy orders, or
        1 if it is a batch of sell orders.

    Note that orders[i] represents a batch of amount_i independent orders with the same price and order type. 
...
Example 1:
    ...
    Explanation: Here is what happens with the orders:
        ...
        - 4 orders of type buy with price 30 are placed. 
            ...
            which is 25 and this sell order is removed from the backlog. 
...
Example 2:
    ...
    Explanation: Here is what happens with the orders:
        ...
        - 1 order of type sell with price 5 is placed. 
            It is matched with the buy order of the highest price, 
        ...
        So the total number of orders = 1999999991, which is equal to 999999984 % (10^9 + 7).
...
class Solution:
    def getNumberOfBacklogOrders(self, orders: List[List[int]]) -> int:
        ...
        return self._getNumberOfBacklogOrders(orders)

    def _getNumberOfBacklogOrders(self, orders: List[List[int]]) -> int:
        """
        Time: beats 95.29%; Space: beats 52.16%
        """
        assert isinstance(orders, list) and len(orders) >= 1
