...
@File    : LC-2090-K-Radius-Subarray-Averages.py
...
"""
LeetCode - 2090 - (Medium) - K Radius Subarray Averages
...
class Solution:
    def getAverages(self, nums: List[int], k: int) -> List[int]:
        ...
        return self._getAverages(nums, k)

    def _getAverages(self, nums: List[int], k: int) -> List[int]:
        assert isinstance(nums, list) and len(nums) >= 1
        assert isinstance(k, int) and k >= 0
        ...
        for i in range(n - (k << 1)):
            res.append((pre_sum[i + k2] - pre_sum[i]) // k2)

        return [-1] * k + res + [-1] * k


def main():
    # Example 1: Output: [-1,-1,-1,5,4,4,-1,-1,-1]
    nums = [7, 4, 3, 9, 1, 8, 5, 2, 6]
    k = 3

    # Example 2: Output: [100000]
    # nums = [100000]
    # k = 0
