...
@File    : LC-0909-Snakes-and-Ladders.py
...
"""
LeetCode - 0909 - (Medium) - Snakes and Ladders
...
class Solution:
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        ...
        # main method: (BFS)
        return self._snakesAndLadders(board)

    def _snakesAndLadders(self, board: List[List[int]]) -> int:
        assert isinstance(board, list) and len(board) >= 2
        n = len(board)

        def id2rc(index: int) -> (int, int):
            r, c = (index - 1) // n, (index - 1) % n
            if r % 2 == 1:
                c = n - 1 - c
            return n - 1 - r, c
