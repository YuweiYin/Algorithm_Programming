...
@File    : LC-1599-Maximum-Profit-of-Operating-a-Centennial-Wheel.py
...
"""
LeetCode - 1599 - (Medium) - Maximum Profit of Operating a Centennial Wheel
...
Description & Requirement:
    ...
    just before the i-th rotation (0-indexed). This means you must rotate the wheel i times before 
    the customers[i] customers arrive. You cannot make customers wait if there is room in the gondola. 
    ...
    If you decide to stop serving customers, all subsequent rotations are free in order to 
    get all the customers down safely. Note that if there are currently more than four customers waiting at the wheel, 
...
Example 1:
    ...
    Explanation: The numbers written on the gondolas are the number of people currently there.
        1. 8 customers arrive, 4 board and 4 wait for the next gondola, 
            the wheel rotates. Current profit is 4 * $5 - 1 * $6 = $14.
...
Example 2:
    ...
    Explanation:
        ...
        2. 9 customers arrive, 4 board and 11 wait (2 originally waiting, 9 newly waiting), 
...
class Solution:
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        assert isinstance(boardingCost, int) and boardingCost >= 1
        assert isinstance(runningCost, int) and runningCost >= 1
        # main method: (process simulation)
        return self._minOperationsMaxProfit(customers, boardingCost, runningCost)

    def _minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        assert isinstance(customers, list) and len(customers) >= 1
        assert isinstance(boardingCost, int) and boardingCost >= 1
        assert isinstance(runningCost, int) and runningCost >= 1
        ...
        profitEachTime = (boardingCost << 2) - runningCost
        if profitEachTime <= 0:
            return res
