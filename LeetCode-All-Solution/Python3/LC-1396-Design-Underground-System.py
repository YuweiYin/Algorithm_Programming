...
@File    : LC-1396-Design-Underground-System.py
...
"""
LeetCode - 1396 - (Medium) - Design Underground System
...
Description & Requirement:
    ...
    double getAverageTime(string startStation, string endStation)
        ...
        There will be at least one customer that has traveled 
            from startStation to endStation before getAverageTime is called.
...
class UndergroundSystem:
    ...
    def checkIn(self, id: int, stationName: str, t: int) -> None:
        # A customer can only be checked into one place at a time.
        self.unfinished[id] = [stationName, t]

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        if id in self.unfinished:
            from_to = (self.unfinished[id][0], stationName)
            if from_to not in self.record:
                self.record[from_to] = [t - self.unfinished[id][1]]
            else:
                self.record[from_to].append(t - self.unfinished[id][1])
            del self.unfinished[id]  # finished traveling
    ...
    def getAverageTime(self, startStation: str, endStation: str) -> float:
        ...
        from_to = (startStation, endStation)
        if from_to in self.record and len(self.record[from_to]) > 0:
            return sum(self.record[from_to]) / float(len(self.record[from_to]))
        else:
            return 0.0


def main():
    # Example 1: Output: [null,null,null,null,null,null,null,14.00000,11.00000,null,11.00000,null,12.00000]
    # command_list = ["UndergroundSystem", "checkIn", "checkIn", "checkIn", "checkOut", "checkOut", "checkOut",
    #                 "getAverageTime", "getAverageTime", "checkIn", "getAverageTime", "checkOut", "getAverageTime"]
    # param_list = [[], [45, "Leyton", 3], [32, "Paradise", 8], [27, "Leyton", 10], [45, "Waterloo", 15],
    ...
    # Example 2: Output: [null,null,null,5.00000,null,null,5.50000,null,null,6.66667]
    command_list = ["UndergroundSystem", "checkIn", "checkOut", "getAverageTime", "checkIn", "checkOut",
                    "getAverageTime", "checkIn", "checkOut", "getAverageTime"]
    param_list = [[], [10, "Leyton", 3], [10, "Paradise", 8], ["Leyton", "Paradise"],
    ...
    for idx in range(1, len(command_list)):
        ...
        if command == "getAverageTime":
            if isinstance(param, list) and len(param) == 2:
                ans.append(obj.getAverageTime(param[0], param[1]))
        elif command == "checkIn":
            if isinstance(param, list) and len(param) == 3:
                obj.checkIn(param[0], param[1], param[2])
            ans.append("null")
        elif command == "checkOut":
            if isinstance(param, list) and len(param) == 3:
                obj.checkOut(param[0], param[1], param[2])
            ans.append("null")
        else:
            ans.append("null")
